...
class DecentralizedDataParallel(Module):
    ...
    def __init__(self,
        ...
        # mixing factor
        self._mixing_factor: float = 1.0

        self._backup_blocks: List[Tensor] = []
        self._backup_params: List[Tensor] = []
    ...
    def set_mixing_factor(self, mixing_factor: float):
        """Set the mixing factor for the communication

        Args:
            mixing_factor (float): Mixing factor for the communication
        """
    ...
    def _ddp_fn(self, _: Tensor, bucket_id: int):
        ...
        if comm_op is not None:
            ...
            if self._param_as_bucket_view:
                ...
                self._param_blocks[bucket_id].mul_(1 - self._mixing_factor)
                self._param_blocks[bucket_id].add_(self._comm_blocks[bucket_id], alpha=self._mixing_factor)
            else:
                ...
                torch._foreach_mul_(self._param_buckets[bucket_id], 1 - self._mixing_factor)
                torch._foreach_add_(self._param_buckets[bucket_id], self._comm_buffers[bucket_id], alpha=self._mixing_factor)
        ...
        self._comm_blocks[bucket_id].div_(len(edge.ranks))
        ...
        if (self._step == 1) and (not self._initialized):
            ...
            with torch.no_grad():
                ...
                for i in range(len(self._param_buckets)):
                    ...
                    self._comm_blocks[i].div_(len(edge.ranks))
    ...
    @torch.no_grad()
    def global_avg(self, may_revert: bool = False):
        ...
        if self._param_as_bucket_view:
            self._backup_blocks = [x.detach().clone() for x in self._param_blocks]
            ...
            self._backup_params = [x.detach().clone() for x in self._params]
    ...
    def revert_global_avg(self):
        """Revert the global average by restoring the backup parameters and blocks"""
        if self._param_as_bucket_view:
            torch._foreach_copy_(self._param_blocks, self._backup_blocks)
        else:
            torch._foreach_copy_(self._params, self._backup_params)
