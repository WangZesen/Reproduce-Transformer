...
criterion = torch.nn.CrossEntropyLoss(ignore_index=tokenizer.token_to_id(SPECIAL_TOKENS.PAD), label_smoothing=cfg.train.label_smoothing)

model = TransformerModule(cfg.data.tokenizer.vocab_size,
...
for idx, batch in enumerate(train_ds):
    src = batch[0].cuda()
    tgt = batch[1].cuda()
    cu_src_lens = batch[2].cuda()
    cu_tgt_lens = batch[3].cuda()
