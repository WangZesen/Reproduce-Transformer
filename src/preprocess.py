...
def clean_worker(src_file: str, tgt_file: str, idx: int):
    ...
    nxtline = "\n"
    assert len(full_src.split(nxtline)) == len(full_tgt.split(nxtline)), (
        f"{len(full_src.split(nxtline))} {len(full_tgt.split(nxtline))}"
    )
...
def convert(files, tokenizer, truncate, out_dir):
    ...
    logger.info(
        f"Percentage of tokens truncated: {sum(1 for x in token_counts if x > truncate) / len(token_counts) * 100:.2f}%"
    )
